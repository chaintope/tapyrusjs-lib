...
export class Transaction {
  ...
  getId(): string {
    ...
    return reverseBuffer(this.getMalFixHash()).toString('hex');
  }
   ...
   */
  private __byteLength(mulFix: boolean): number {
    return (
      8 +
  ...
  private __toBuffer(
    ...
    if (!buffer)
      buffer = Buffer.allocUnsafe(this.__byteLength(mulFix)) as Buffer;
